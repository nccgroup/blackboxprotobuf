...
@given(typedef = strategies.message_typedef_gen(), name = st.from_regex(protofile.NAME_REGEX))
...
def test_proto_export(tmp_path, typedef, name):
    ...
    with tempfile.NamedTemporaryFile(mode='w', dir=str(tmp_path), suffix='.proto', delete=True) as outfile:
        ...
        os.mkdir(py_out)
        outfile.flush()
        subprocess.check_call('/usr/bin/protoc --python_out ./py_out %s' %  os.path.basename(outfile.name), shell=True, cwd=str(tmp_path))
...
def test_proto_export_inverse(tmp_path, x, name):
    ...
    typedef, message = x
    with tempfile.NamedTemporaryFile(mode='r+', dir=str(tmp_path), suffix='.proto', delete=True) as outfile:
            ...
            for field_num in typedef1.keys():
                ...
                if typedef1[field_num]['type'] == 'message':
                    ...
                    if 'message_typedef' in typedef1[field_num]:
                        ...
                        message_typedef1 = typedef_map[typedef1[field_num]['message_type_name']]
                    if 'message_typedef' in typedef2[field_num]:
                        ...
                        assert(typedef2[field_num]['message_type_name'] in new_typedef_map)
                        message_typedef2 = new_typedef_map[typedef2[field_num]['message_type_name']]
        ...
        decode_forward, _, _ = length_delim.decode_message(encode_forward, new_typedef_map[name])
        decode_backward, _, _ = length_delim.decode_message(encode_backward, typedef_map[name])
...
def test_proto_decode(tmp_path, x, name):
    ...
    with tempfile.NamedTemporaryFile(mode='w', dir=str(tmp_path), suffix='.proto', delete=True) as outfile:
        ...
        os.mkdir(py_out)
        outfile.flush()
        subprocess.check_call('/usr/bin/protoc --python_out ./py_out %s' %  basename, shell=True, cwd=str(tmp_path))
        ...
        decoded_message = google.protobuf.json_format.MessageToDict(my_message, including_default_value_fields=True)

        note(message)
        note(decoded_message)
        note(google.protobuf.json_format.MessageToJson(my_message, including_default_value_fields=True))

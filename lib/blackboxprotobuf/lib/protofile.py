    ...
    for field_number, field_typedef in typedef.items():
        ...
        if field_typedef['type'] == 'message':
            ...
            if 'alt_typedefs' in field_typedef:
                ...
                _print_message(proto_type, field_typedef['message_typedef'], output_file, depth+1)
        else:
            if field_typedef['type'] not in PROTO_FILE_TYPE_MAP:
                raise TypedefException("Type %s does not have a mapping to protobuf types." % field_typedef['type'])
        ...
        output_file.write(indent)
        output_file.write(u'  %s%s %s = %s%s;\n' % ('repeated ' if is_repeated else '', proto_type, field_name, field_number, field_options))
...
def import_proto(input_string=None, input_filename=None, input_file=None):
    ...
    while line:
        ...
        if line.startswith("syntax") and SYNTAX_REGEX.match(line):
            ...
            logging.warn("Proto file has import which is not supported "
                         "by the parser. Ensure the imported files are "
                         "processed first: %s", line)
    ...
    for tree in message_trees:
        _parse_message(tree, typedef_map, message_names, enum_names, package_prefix, syntax_version == "proto3")
...
def _check_message_name(current_path, name, known_message_names):
    ...
    logging.debug("Message %s not found from %s Known names are: %s", name, current_path, known_message_names)
    return None

def _parse_message(message_tree, typdef_map, known_message_names, enum_names, prefix, is_proto3):
    message_typedef = {}
    ...
    for line in message_tree['data']:
        ...
        if match:
            field_number, field_typedef = _parse_field(match, known_message_names, enum_names, prefix, is_proto3)
    ...
    for inner_message in message_tree['inner_messages']:
        ...
        _parse_message(inner_message, typdef_map, known_message_names, enum_names, prefix, is_proto3)
...
def _parse_field(match, known_message_names, enum_names, prefix, is_proto3):
    ...
    if not bbp_type:
        ...
        raise ValueError("Could not get a type for field %s: %s" % (field_name, field_type))
    ...
    if is_packed and bbp_type not in PACKABLE_TYPES:
        raise ValueError("Field %s set as packable, but not a packable type: %s" % (field_name, bbp_type))
    if is_packed:
